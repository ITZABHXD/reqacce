...
@User.on_message(filters.command(["accept", "approve"], C) & authchat)                     
async def approve(client: User, message: Message):
    ...
    try:
       ...
       print(e)
    hhh = await client.send_message(chat.id, "Accept Krdiya Vmro 🙃")
...
async def decline(client: User, message: Message):
    ...
    try:
       ...
       print(e)
    hhh = await client.send_message(chat.id, "Decline Krdiya Vmro 🥲")  
    await asyncio.sleep(3)
    await hhh.delete()     
